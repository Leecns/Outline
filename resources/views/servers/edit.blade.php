...
@section('content')
    <section class="mt-5 px-5">
        <header class="d-flex justify-content-between align-items-center">
            <h2 class="m-0">{{ __('Editable information') }}</h2>
            <a class="btn" href="{{ route('servers.keys.index', $server->id) }}">{{ __('Back') }}</a>
        </header>
        ...
        <form class="d-grid gap-3 mt-3" action="{{ route('servers.update', $server->id) }}" method="post">
            ...
            <section>
                ...
                @error('name')<small class="ps-1 text-danger">{{ $message }}</small>@enderror
            </section>
            ...
            <section>
                ...
                @error('hostname_for_new_access_keys')<small class="ps-1 text-danger">{{ $message }}</small>@enderror
            </section>
            ...
            <section>
                ...
                <small class="ps-1 d-block text-muted">{{ __('This will not affect the existing access keys.') }}</small>
                @error('port_for_new_access_keys')<small class="ps-1 text-danger">{{ $message }}</small>@enderror
            </section>
